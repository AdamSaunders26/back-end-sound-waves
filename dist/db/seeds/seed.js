...
const seed = ({ boardsData, usersData, wavesData, commentsData }) => {
    ...
    CREATE TABLE comments (
      ...
      wave_id INT REFERENCES waves (wave_id) NOT NULL
        ...
        .then(() => {
        const insertUsersData = format("INSERT INTO users (username, email, avatar_url, password) VALUES %L RETURNING * ", usersData.map(({ username, email, avatar_url, password = "$2a$04$H.8.1IEF5nfICzTo8TNN9OrqSj9.9egqZSm01MKkzO3Sgph12hDOu", }) => [username, email, avatar_url, password]));
        ...
        .then(() => {
        const insertWavesData = format("INSERT INTO waves ( title, wave_url, created_at, user_id, board_id, transcript) VALUES %L RETURNING * ", wavesData.map(({ title, wave_url, created_at, user_id, board_id, transcript }) => [
            title,
            wave_url,
            created_at,
            user_id,
        ...
        .then(() => {
        const insertCommentsData = format("INSERT INTO comments ( comment, created_at,user_id, wave_id) VALUES %L RETURNING * ", commentsData.map(({ comment, created_at, user_id, wave_id }) => [
            comment,
            created_at,
            user_id,
            wave_id,
